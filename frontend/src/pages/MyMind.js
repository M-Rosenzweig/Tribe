...
function MyMind({ user, tribes }) {
  ...
  useEffect(() => {
    if (energy === 1) {
      ...
      setFeeling("Decent");
      setTagline(
        "I am feeling Decent but can use some attention from my tribe members"
      );
    } else if (energy === 3) {
    ...
    <div className="myMindMaster">
      <div className="PrioritiesSection">
        ...
        <form onSubmit={uploadPriority}>
          ...
          ></input>
          <button className="ClickPriority">
            <BiSend />
          </button>
        </form>
      </div>
      ...
      <div className="WorriesSection">
        <h1 className="WorriesTitle">My Blockers</h1>
        {mappedWorries}
        <form onSubmit={uploadWorry}>
          <input
            type="text"
            value={worry}
            onChange={handleWorry}
            className="PriorityInput"
            placeholder="type here"
          ></input>
          <button className="ClickPriority">
      ...
      <div className="EnergySection">
        <h2>
          <span id={flip ? 'flip-horizontal-bottom' : 'flip-horizontal-top' } className="dot EnergyWeak  "></span>
          <span id={flip ? 'flip-horizontal-bottom' : 'flip-horizontal-top' } className="dot EnergyDecent "></span>
          <span id={flip ? 'flip-horizontal-bottom' : 'flip-horizontal-top' } className="dot EnergySolid "></span>
          <span id={flip ? 'flip-horizontal-bottom' : 'flip-horizontal-top' } className="dot EnergyAwesome  "></span>
        </h2>
        <button
          id="EnergyButton"
          className={className}
